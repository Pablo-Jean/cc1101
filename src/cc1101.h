/*
 * cc1101.h (former name rf_driver.h)
 *
...
/* define ------------------------------------------------------------*/
#define CRYSTAL_MHZ(x)				x/1000000
#define CRYSTAL_KHZ(x)				x/1000
...
#ifndef US_TO_SEC
#define SEC_TO_US(x)				(MS_TO_US(x*1000))
#endif
...
#ifndef MHZ_TO_HZ
#define MHZ_TO_HZ(x)				(KHZ_TO_HZ(1000*x))
#endif
...
#define MAX_CHANNEL_SPACING(x) 		(0xFF+256)*x*(1<<3)/(1<<18)
#define MIN_CHANNEL_SPACING(x) 		(0x0+256)*x/(1<<18) //Min value possible, but min legal value?
...
}cc1101_frame_status_e;


/* Functions typedefes ----------------------------------*/

typedef void (*cc1101_gpio_control_fxn_t)(bool state);
typedef void (*cc1101_delay_us_fxn_t)(uint32_t us);
typedef int8_t (*cc1101_spi_data_fxn_t)(uint8_t *data, uint32_t len);

/* Structs -----------------------------------------------*/

typedef struct{
	uint32_t u32XtalFreqMhz;
	cc1101_modulation_e eModulation;
	cc1101_ismband_e eIsmBand;
	cc1101_gpio_control_fxn_t fxCsGpio;
	cc1101_spi_data_fxn_t fxSpiWrite;
	cc1101_spi_data_fxn_t fxSpiRead;
	cc1101_delay_us_fxn_t fxDelayUs;
}cc1101_params_t;

typedef struct{
	uint8_t* frame_buffer;
	uint16_t* len;
	uint8_t data_len_loc;
	uint8_t data_len_size;
	uint8_t* lqi;
	uint8_t* rssi;
}cc1101_frame_data_t;
...
typedef struct{
	...
	// internal usage (the previous aren't too??)
	bool gdo0_flag;
	int32_t timeout;
}cc1101_t;
...
/* Driver Config -------*/
void cc1101_set_modulation_mode(cc1101_t *cc1101, cc1101_modulation_e mod);
void cc1101_set_ISMband(cc1101_t *cc1101, cc1101_ismband_e band);
void cc1101_set_channel(cc1101_t *cc1101, uint8_t channel);
void cc1101_set_output_power_level(cc1101_t *cc1101, int8_t power);
void cc1101_set_syncword(cc1101_t *cc1101, uint16_t syncword);
//packet control
void cc1101_set_addr(cc1101_t *cc1101, uint8_t addr);
...
float cc1101_set_channel_spacing(cc1101_t *cc1101, float cpsacing);
